...
class HiringCalibrator:
    ...
    def _analyze_bias_patterns(
        self, data: List[Dict[str, Any]], target_competencies: Optional[List[str]]
    ) -> Dict[str, Any]:
    ...
    def _analyze_demographic_bias(
        ...
        return {
            ...
            "recommendation": (
                self._generate_demographic_bias_recommendation(demographic, bias_details)
                if bias_detected
                else None
            ),
        }
            ...
            "trends": {
                "score_trend": {
                    "direction": (
                        "increasing"
                        if score_trend > 0.01
                        else "decreasing" if score_trend < -0.01 else "stable"
                    ),
                ...
                "hire_rate_trend": {
                    "direction": (
                        ...
                        else "decreasing" if hire_rate_trend < -0.005 else "stable"
                    ),
