...
class QualityReport:
    ...
    def _calculate_summary_stats(self):
        ...
        self.summary_stats = {
            "highest_dimension": (
                max(self.dimensions.items(), key=lambda x: x[1].score)[0] if scores else "None"
            ),
            "lowest_dimension": (
                min(self.dimensions.items(), key=lambda x: x[1].score)[0] if scores else "None"
            ),
            "score_variance": (
                sum((score - self.overall_score) ** 2 for score in scores) / len(scores)
                if scores
                else 0
            ),
...
class QualityScorer:
    ...
    def _score_directory_structure(self, dimension: QualityDimension):
        ...
        dimension.add_score("directory_structure", score, 25, f"Directory structure completeness")
