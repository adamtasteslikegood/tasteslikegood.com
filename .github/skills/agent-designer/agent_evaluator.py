...
class AgentEvaluator:
    ...
    def _analyze_tool_usage(self, logs: List[ExecutionLog]) -> Dict[str, Dict[str, Any]]:
        ...
        for tool, stats in tool_stats.items():
            result[tool] = {
                ...
                "error_rate": (
                    stats["error_count"] / stats["usage_count"] if stats["usage_count"] > 0 else 0
                ),
                "avg_duration": (
                    stats["total_duration"] / stats["usage_count"]
            ...
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    estimated_performance_gain=(
                        list(bottleneck.estimated_improvement.values())[0]
    ...
    def generate_report(self, logs: List[ExecutionLog]) -> EvaluationReport:
        ...
        summary = {
            "evaluation_period": {
                "start_time": (
                    min(log.start_time for log in logs if log.start_time) if logs else None
                ),
                "end_time": max(log.end_time for log in logs if log.end_time) if logs else None,
                "total_duration_hours": (
                    system_metrics.total_tasks / system_metrics.throughput_tasks_per_hour
