    ...
    def _analyze_constraint_changes(
        ...
        for constraint_type in ["primary_key", "foreign_key", "unique", "check"]:
            ...
            before_set = (
                ...
                else {str(before_constraint)} if before_constraint else set()
            )
            after_set = (
                set(str(c) for c in after_constraint)
                if isinstance(after_constraint, list)
                else {str(after_constraint)} if after_constraint else set()
            )
    ...
    def _build_compatibility_report(
        ...
        risk_assessment = {
            "overall_risk": (
                "high"
                if breaking_count > 0
                else "medium" if potentially_breaking_count > 0 else "low"
            ),
            "deployment_risk": (
                "requires_coordinated_deployment"
                if breaking_count > 0
                else "safe_independent_deployment"
            ),
            "rollback_complexity": (
                "high" if breaking_count > 3 else "medium" if breaking_count > 0 else "low"
            ),
        ...
        return CompatibilityReport(
            schema_before=(
                json.dumps(before_schema, indent=2)[:500] + "..."
                if len(json.dumps(before_schema)) > 500
                else json.dumps(before_schema, indent=2)
            ),
            schema_after=(
                json.dumps(after_schema, indent=2)[:500] + "..."
