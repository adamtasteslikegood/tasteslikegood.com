...
- [x] `ci.yml`: change `on.push.branches` / `on.pull_request.branches` to `[main, dev]`
      — verify: open a scratch PR into `dev`; Lint/Type-Check/Test/Security jobs appear
- [x] `ci.yml`: add `concurrency` group with `cancel-in-progress: true`
      — verify: `actionlint .github/workflows/ci.yml` clean; force-push a PR twice, first run cancels
- [x] `ci.yml`: replace deprecated `safety check` with `pip-audit` (keep
      `continue-on-error: true`)
      — verify: security job runs `pip-audit` and completes
- [x] Note in scratch-PR description which jobs are red (expected: lint, type-check, test)

## Phase 2 — Fix the tree (one PR per bullet)

- [x] Run `uv run black .`, commit the 15-file sweep alone; add the commit hash to
      `.git-blame-ignore-revs`
      — verify: `uv run black --check .` exits 0
- [x] Fix 38 flake8 violations; align flake8 config with Black
      (`max-line-length = 100`, `extend-ignore = E203,W503`); targeted `# noqa` only
      with justification
      — verify: `uv run flake8 . --count` prints 0
- [x] Fix mypy crash: `explicit_package_bases = true` (or exclude `scripts/`) in
...
- [x] Root-cause the 4 `tests/test_public_ssr.py` failures (canonical link + sitemap).
...
- [x] Close the loop: re-run the scratch PR from Phase 1 — all jobs green

## Phase 3 — Build gate

- [x] Add `build` job to `ci.yml`: `docker build -f Dockerfile .` (no push, no creds)
...
- [x] Retune `gemini-scheduled-triage.yml` cron hourly → every 6h; add a dedicated
...
- [x] Standalone `gemini-triage.yml` on `issues: opened`, reusing
...
- [x] Standalone `gemini-review.yml` triggered by `gemini-review` label +
...
- [x] Standalone `gemini-invoke.yml` on `@gemini-cli` comments, **author-association
...
- [x] Update `docs/ci/CI-AUDIT-REPORT.md` (or supersede it with a dated addendum)
