...
def get_all_branches(repo_root):
    """
    Get a list of all remote branch names in the repository.
    ...
    """
    try:
        # Get only remote branches (origin/)
        result = subprocess.run(
            ["git", "branch", "-r"], cwd=repo_root, capture_output=True, text=True, check=False
        )
        branches = []
...
def scan_branch(repo_root, branch_name, skip_git=True):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            # Only use origin/ prefix for remote branches
            ...
            if check_result.returncode != 0:
                result["checkout_error"] = (
                    f"Remote branch 'origin/{branch_name}' not found (only remote branches are scanned)"
                )
                return result
        ...
        if non_ascii_paths:
            ...
            if dirs:
                ...
                for path in dirs:
                    ...
                    if non_ascii_chars:
                        f.write(
                            f"  - Non-ASCII characters: {', '.join(repr(c) for c in non_ascii_chars)}\n"
                        )
                f.write("\n")
            ...
            f.write(
                "Consider renaming these paths to use only ASCII characters (A-Z, a-z, 0-9, hyphens, underscores).\n"
            )
        else:
            f.write("## Results\n\n")
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for result in branch_results:
            if result["checkout_error"]:
                ...
                f.write(
                    f"| {result['branch']} | {result['scanned']} | {result['successful']} | "
...
Examples:
  ...
  %(prog)s --all-branches --include-git-dir   # Scan all branches including .git
    ...
    parser.add_argument("--branch", type=str, help="Scan a specific branch")
    parser.add_argument(
        "--branches", type=str, help="Scan multiple branches (comma-separated list)"
    )
    parser.add_argument(
        "--all-branches", action="store_true", help="Scan all branches in the repository"
    )
